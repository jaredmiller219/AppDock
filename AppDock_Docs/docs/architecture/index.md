...
- **Popover UI**: `MenuController` builds the popover host, with `PopoverContentView` handling layout and `DockView` rendering the grid and menu rows.
- **Dock Composition**: `DockView` uses `DockAppList` to shape entries and `Dock/Parts` for subviews.
